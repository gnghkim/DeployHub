...
export function SuggestionForm({
  ...
  return (
    <form action={formAction} className="flex min-w-0 flex-wrap items-center gap-2">
      ...
      <label className="min-w-0 max-w-full text-xs text-[var(--annotation)]">
        구성요소
        <select
          name="componentId"
          required
          className="mt-1 block h-9 min-w-0 max-w-full rounded-[var(--radius-button)] border border-[var(--rule)] bg-[var(--paper)] px-3 text-sm text-[var(--line)] sm:ml-2 sm:mt-0 sm:inline-block"
        >
