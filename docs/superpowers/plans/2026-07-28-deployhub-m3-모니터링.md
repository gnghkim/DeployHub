...
**선행:** M2 완료(`docs/superpowers/plans/2026-07-27-deployhub-m2-뒷단파악.md`), 구축방안 §7.3·§18.3
...
Overview에 판정별 프로젝트 수를 더한다. M2에서 만든 카드 옆에 붙인다.

프로젝트 상세에는 그 프로젝트의 판정과 근거가 된 이벤트를 보여준다. **판정만 보여주고 근거를 감추지 마라** — 왜 장애인지 모르면 화면이 쓸모없다.

- [ ] **Step 3: 검증과 커밋**

**게이트 통과 조건:** 판정이 파생 계산일 것. 대상별로 해소를 판단할 것.
