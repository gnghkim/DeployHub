...
- `basis: 'repository'` → `component_resources.linked_by = 'repository'`로 즉시 연결 가능(사용자가 직접 적은 값이므로)
- `basis: 'name'` → `linked_by = 'suggested'`로 저장하고 확인 대기 상태로 둔다
- 사용자가 화면에서 확정하면 `linked_by = 'user'`로 승격한다
